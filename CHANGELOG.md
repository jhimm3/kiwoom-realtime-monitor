...
## 2026-09-25 Google Drive 시작 전 엄격 복원

- 사용자가 명시적으로 예약한 Drive 설정·테마·뉴스 AI 복원은 자료와 SHA-256을 앱 데이터에 고정하고 다음 시작의 DB 초기화·중앙 동기화 전에 적용한다. 지원 앱 프로세스가 공유하는 파일 잠금 아래 두 SQLite DB와 설정 자산의 변경 전 사본을 보존하고, 적용 중 오류·강제 종료 뒤에는 다음 시작에 원상복구한다. 복원 예약 뒤 자동 업로드·다운로드는 재시작까지 보류한다. 기존 즉시 다운로드와 NAS 서버 복원은 변경하지 않는다.

## 2026-09-25 Google Drive 복원 실패·완료 신호

- Drive 복원 결과에 작업 종류·상태·로컬 적용 여부를 담아 성공 문구 문자열 검사에 의존하지 않는다. 설정 자산은 사전 검증 뒤 개별 원자 교체하며 실패를 명시한다. 이미 설정 DB가 commit된 뒤 자산 쓰기가 실패하면 실패 알림과 함께 앱 화면·메모리를 저장된 DB에 다시 맞춘다.

## 2026-09-25 Google Drive 설정·테마 묶음 적용

- Drive에서 설정과 테마를 함께 받으면 두 문서를 모두 검증한 뒤 `monitor.sqlite3`에 한 트랜잭션으로 반영한다. 뉴스 AI 파일도 설정·테마 적용 전에 형식을 검증한다. 뉴스 AI DB와 파일 설정·자산은 별도 저장 경계다.

## 2026-09-25 Google Drive 설정·테마 DB 적용 원자화

- 설정·테마 프로필·테마 화면설정을 `monitor.sqlite3`의 단일 트랜잭션으로 적용한다. 이 DB 안에서 저장 오류가 발생하면 앞서 적용한 설정이나 테마도 함께 롤백한다. 별도 뉴스 AI DB와 파일 설정·이미지는 이 트랜잭션 범위 밖이다.

## 2026-09-25 Google Drive 복원 파일 사전검증

- 설정·테마·뉴스 AI 백업의 형식과 읽기 가능 여부를 모두 확인한 뒤 로컬 복원을 시작한다. 테마 데이터가 빠진 현재 형식의 백업은 기존 테마를 지우기 전에 거부한다.

## 2026-09-25 Google Drive 백업 다운로드 적용 순서

- 설정·테마·뉴스 AI 파일 다운로드를 모두 완료한 뒤 로컬 복원을 시작한다. 중간 다운로드 실패 시 일부 파일만 복원되지 않는다. 뉴스 AI 백업의 UTF-8 읽기 오류는 백업 오류로 표시한다.

## 2026-09-25 뉴스 AI·매매일지 백업 파일 저장 보존

- 뉴스 AI JSON과 매매일지 SQLite 백업도 임시 파일을 완성한 뒤 교체한다. 교체 실패 때 기존 백업을 보존하고 임시 파일을 정리한다.

## 2026-09-25 설정·테마 백업 파일 저장 보존

- 설정·테마 백업을 같은 폴더의 임시 파일에 완전히 기록한 뒤 교체한다. 교체에 실패하면 기존 백업 파일을 유지하고 임시 파일을 정리한다.

## 2026-09-25 백업 파일 오류 처리 보완

- 설정·테마 백업 파일의 UTF-8 디코딩 실패를 복원 오류로 표시한다. 설정 백업의 뉴스 표시 열 값이 잘못된 자료형이면 현재 표시 열을 유지해 공통 설정 적용 뒤 복원이 중단되지 않게 한다.

## 2026-09-25 설정 백업 복원 확인창 오류 수정

- 설정 백업 불러오기에서 존재하지 않는 대화상자 변수를 참조해 확인창을 열기 전에 발생하던 `NameError`를 수정했다. 확인 후 복원과 취소 경로를 검사했다.

## 2026-09-25 B01~B07 감사 보완 (로컬 검증)

- 테마 원격 스냅샷 적용 시 로컬 편집 revision을 같은 SQLite 교체 트랜잭션에서 재확인한다. 충돌하면 로컬 편집을 보존하고 재동기화한다.
- 미완료 분봉·일봉 coverage는 키움 차트 아카이브로 재사용하지 않는다. 수급 원본 저장 실패·빈 응답, 시장지수 대상일 자료 부재·연속조회 미종료는 완료 표식을 쓰지 않는다.
- NXT 여부는 당일 관측만 재사용하고 서버의 실시간 유실 통지를 PC의 부분 자료 표시·진단에 연결한다.
- B03은 현재 요청 조건상, B08은 현 우선순위상 제외한다. 예정 서버 빌드: `2026.09.25-audit-b01-b07-v1`.

## 2026-09-25 TOP20 품질·재시도·잠금 경계 (로컬 검증)

- TOP20 분 수집이 분 중간에 시작되거나 수집 루프가 2초 넘게 비거나 실시간 구독자 큐에서 이벤트가 유실되면 완료 분도 `partial`로 기록한다. 큐 유실 건수를 계측하고 데스크톱 WebSocket에 `realtime_gap`을 알린다.
- NXT 일시 조회 실패는 확인된 비대상으로 캐시하지 않는다. 장후 백필은 전체 작업 성공 후에만 당일 완료로 표시하며 실패 시 제한된 간격으로 재시도한다.
- REST의 PostgreSQL 캐시 조회와 응답 저장을 공통 guard 밖으로 옮기고, 동일 요청은 inflight를 공유하며 재진입 시 소유권·자격증명 상태를 재확인한다. TOP20 지수 collector lock 경계(A10)는 사용자 결정에 따라 이번 변경에서 제외했다.
- 예정 NAS 빌드: `2026.09.25-top20-quality-lock-v1`. 운영 지연과 유실률의 전후 측정은 컨테이너 재빌드 뒤 필요하다.

## 2026-09-25 거래대금·백필 완료 판정·TOP20 마감 보존 (로컬 검증)

- 0B 누적 거래대금 필드가 잠시 비었다 돌아오면 그 사이 추정한 금액을 실제 누적 증가분과 대조해 중복 합산을 제거한다. 과대 추정분은 해당 분봉에서 보정한다. 분 경계를 지난 뒤 복귀한 경우 원시 틱별 실제 금액을 알 수 없어 분별 귀속은 추정으로 남는다.
- 중앙 REST 결과에 원본 응답 저장 성공 여부를 내부적으로 구분한다. 순위 화면 조회의 성공은 유지하면서 NAS 분봉 백필은 응답 봉의 실제 저장을 확인한 뒤에만 coverage를 완료로 기록한다. 대상일 봉이 없는 경우는 미완료로 남긴다.
- TOP20 완성 분은 membership·당일 편입 DB 쓰기 전에 재시도 outbox에 보존한다. 예정 NAS 빌드: `2026.09.25-top20-recording-v1`.

## 2026-09-25 미사용 ka10016 자동 수집 제거 (로컬 검증)

- NAS의 매분 5·20·250일 시장 전체 신고가 목록 조회와 PC의 초기 목록 후속 조회를 제거했다. 숨겨진 목록 새로고침 버튼과 전용 worker도 제거했다.
- 신고가 가격·거리·강조는 종목별 수정 일봉과 실시간 `0B`를 계속 사용한다. 기존 저장 목록·스냅샷은 보존해 과거 자료 조회 계약을 유지한다.
- 예정 NAS 빌드: `2026.09.25-no-ka10016-v1`.

## 2026-09-25 순위 후속 시작·NAS 신고가·짧은 캐시 (로컬 검증)

- PC의 NXT 확인 완료·실시간 구독 승인·5초 fallback은 현재 적용한 순위 회차의 후속 보완을 한 번만 시작한다. 우선순위 준비 중 건너뛴 신호와 이전 순위의 늦은 구독 승인은 완료로 표시하지 않는다.
- NAS `ka10016`의 5·20·250일 목록은 요청 시장이 KRX(`stex_tp=1`)임을 확인해 평일 09:00~15:29에는 매분 조회하고 16:00~19:59에는 실행 중 하루 한 번만 추가 조회한다. 야간·주말 반복 조회를 멈춘다.
- `ka00198`의 0.2초와 2초 이하 TR 결과 캐시는 RAM에서만 재사용한다. PostgreSQL query cache 읽기·쓰기를 생략하지만 별도 원본 시장자료 response handler 저장은 유지한다. 장기 TTL 캐시는 그대로다. connection pool과 TOP20 트랜잭션 경계는 변경하지 않았다.
- NAS 예정 빌드: `2026.09.25-followup-newhigh-cache-v1`. 운영 성능 개선은 재빌드·재기동 후 동일 지표로 검증해야 한다.

## 2026-09-25 TOP20 시장 분류 보정 종료 대응 (로컬 검증)

- TOP20 시장 분류 보정이 전체 후보를 `fetchall()`로 먼저 읽지 않고 행별로 처리하며, 종료 요청이 오면 다음 행을 시작하기 전에 중단한다. 이미 보정한 행은 commit하고 다음 실행에서 남은 후보를 다시 확인한다.

## 2026-09-25 TOP20 신규 종목 0B 조기 구독 (로컬 검증)

- NAS가 검증된 TOP20 순위를 메모리에 게시할 때 같은 종목 목록으로 KRX 0B 구독을 요청한다. 순위·편입·지수 DB 저장 대기와 신규 종목의 구독 시작을 분리하고, 실제 수신 준비 표시는 WebSocket REG 승인 이후로 유지한다.

> 현재 릴리스: **2.1.0 (2026-09-21)** — 아래 2026-09-15~21 항목과 관련 구현 보고서를 포함한다.

## 2026-09-25 뉴스 DB 이관 완료 기록

- 앱 시작 시 뉴스 스키마 v4 기록이 완전하면 실행기를 다시 돌리지 않고 버전만 확인한다. 메인 DB의 옛 뉴스 표를 전용 DB로 옮기는 일회성 작업은 복사 확인·원본 제거가 끝난 뒤 `legacy_news_transfer_migrations` v1로 기록해 이후 시작에서 건너뛴다. 완료 후 뉴스 DB가 사라지거나 옛 표가 다시 나타나면 조용히 빈 DB를 만들거나 재이관하지 않고 오류를 낸다. 현재 PC DB는 뉴스 스키마 v4가 기록돼 있고 옛 메인 DB 뉴스 표는 없다.

## 2026-09-25 TOP20 통계 날짜별 저장 (로컬 검증)

- PC와 NAS 통계에서 완료된 날짜의 시간대·일별 집계를 저장해 기간 전환과 재시작 때 원본 분 행 전체를 다시 합산하지 않는다. 원본 분·시장 자료가 보완되면 해당 날짜만 재계산하고 당일은 새 분을 반영한다. 거래가 없는 공휴일의 0원 TOP20 행은 평균과 일별 비교에서 제외한다. 시간대별 통계에 순위를 붙이고 PC 자동 수집은 앱 실행 중에만 진행된다는 안내를 표시한다. 서버 빌드 식별자는 `2026.09.25-top20-statistics-cache-v1`; 운영 반영은 재빌드 뒤 `/health.server_build` 확인이 필요하다.

## 2026-09-25 TOP20 시장 구분 보완 시작 경로

- 앱 시작 중 메인 UI 스레드가 `repair_top20_market_splits()`를 직접 실행하던 중복 호출을 제거했다. 기존 저우선순위 QThread가 시작 직후 동일한 보완을 수행하고 완료되면 TOP20 표시를 갱신한다. 저장 알고리즘과 NAS 서버 코드는 변경하지 않았다.

## 2026-09-25 과거 뉴스 수집기 모니터 응답 표시

- 실행 중인 수집기의 heartbeat가 오래됐을 때 프로세스 사망과 응답 지연을 구분해 표시한다. PC BODY/RULE 건수 집계가 실제 약 16.6초 걸리는데 10초마다 시작하던 모니터 주기를 60초로 조정해 반복 CPU 부하를 줄였다. 수집 요청·원장 저장 정책은 변경하지 않았다.

## 2026-09-25 모의 자동운용 지연 계측 (로컬 검증)

- 모의 자동운용 상태에 최근 poll·관측 DB 조회·관측당 최대 처리 시간과 마지막 분봉의 이용 가능 시각부터 처리 완료까지의 지연을 추가하고 앱 상태 화면에 표시한다. 시각 누락·미래 분봉은 정상 지연으로 계산하지 않고 별도 표시한다. 주문 판단·저장 checkpoint는 변경하지 않았다. NAS 빌드 대상은 `2026.09.25-mock-runtime-latency-v1`이며 운영 반영과 장중 실측은 별도 확인이 필요하다.

## 2026-09-25 과거 원주가/NAS 분봉 대조

- D03 월별 동결 입력을 종가 단일가 시각 계약으로 새 ID에 재생성하고 TRAIN·VALIDATION 개발 패키지와 준비도를 재검증했다. 기존 불변 v1 세션 프로필도 정확한 구계약으로 읽을 수 있게 호환성을 복구했다. 실데이터 구조 실행에서 장마감 미체결 주문이 다음 거래일의 다른 종목 봉을 건너뛰며 여러 달의 후속 주문을 막는 원인을 확인해, 다음 세션 첫 봉에서 주문을 검열하고 예약 현금을 해제하도록 수정했다. 다른 종목 봉은 주문 체결 revision 근거로 기록하지 않는다. 최종 동일 구현으로 옛/새 TRAIN 눌림을 재실행해 67체결·50종료 거래·순손익 -1,976원 및 체결 사건 순서 일치를 확인했다. 수정 입력의 추가 장마감 주문 1건은 검열됐다. OOS와 운영 DB는 변경하지 않았다.
- 수능일 지연 개장(2021~2025년 확인된 다섯 날짜)에는 KRX 정규장이 10:00~16:30이다. CREON 15:30을 일반 봉, 16:30을 종가 단일가로 판정하도록 공통 거래일 시간표·D03 재생·1분/NAS·5분 시각 감사를 변경했다. 2025-11-13 `347850` 379개 원분봉의 중복시각 오류가 사라졌고, 2021·2022·2023년 삼성전자 5분봉 표본은 각 77개 기대 시각과 일치했다.
- CREON 원주가 정규장 1분봉과 NAS 저장 키움 KRX 분봉을 읽기 전용으로 대조하는 도구를 추가했다. 보통의 CREON 종료시각은 1분 앞선 NAS 시작시각과 맞추되 15:30 종가 단일가 체결은 양쪽의 15:30과 직접 맞춘다. `035720` 2026-09-22의 381개 정규장 OHLCV가 모두 일치해 기존 15:29 누락 판정을 정정했다.
- CREON 원주가 5분봉의 09:05~15:20 종료시각과 별도 15:30 마감 체결, 날짜별 1분/5분 비혼합을 읽기 전용 감사한다. D03 역사 1분 재생에서는 15:30 체결을 가상의 15:29~15:30 연속 봉이 아니라 15:20~15:30 종가 단일가 단계로 표시한다. 기존 동결 입력과 운영 DB는 변경하지 않으며 새 성과 비교에는 파생 입력을 재생성해야 한다.

## 2026-09-25 연구 NAS 빈 날짜 재확인 (로컬 검증)

- NAS 조회 당시 관측이 0건인 평일은 다음 날짜로 진행하면서 별도 재확인 원장에 기록한다. 하루 뒤 다시 확인하고 여전히 비어 있으면 하루 뒤로 미루며, 늦게 들어온 자료는 기존 동결·검증·불변 게시 경로로 등록한 후 해당 원장에서 제거한다. 새 날짜 진행 cursor는 되돌리지 않고 과거 재확인은 10분에 최대 한 번으로 제한한다. 연구 DB v27이 이 원장을 추가한다.

## 2026-09-25 연구 NAS 새 날짜 자동 준비 (로컬 검증)

- 거래일 확장과 NAS 자동 준비를 함께 켜면 기준 단일 일별 입력 다음 날짜부터 완료 후 24시간이 지난 날짜를 한 번에 하루씩 조회한다. 주말·NAS 관측 0건 날짜는 건너뛰고, 자료가 있는 날짜는 기존 동결 export·검증·게시·등록 경로를 사용한다. 게시 후 등록이 실패해도 다음 실행에서 완성 파일을 먼저 복구하며, 연구 DB v26은 작업자 소유권으로 보호한 다음 날짜 cursor를 저장한다. 기존 고정 범위 자동 준비는 유지한다. 실제 NAS 운영 장시간 검증은 아직 하지 않았다.

## 2026-09-25 연구 일별 평가기간 확장 (로컬 검증)

- 지속 연구의 새 자료 폴더에 opt-in 거래일 확장을 추가했다. 새 날짜의 완성된 일별 입력을 발견하면 단일 TRAIN/VALIDATION 평가 구간만 같은 일수만큼 이동해 별도 실험을 등록한다. 전략·비용·예산과 기존 동결 입력은 유지하며 FINAL/OOS, 거래 근거가 없는 날짜, 비용 유효기간 밖 입력은 자동 등록하지 않는다. 연구 DB v25가 source 설정에 `rolling_daily`를 추가한다. NAS의 신규 날짜 export 자동화와 운영 장시간 검증은 아직 하지 않았다.

## 2026-09-24 과거 뉴스 PC BODY/RULE 전처리 (로컬 검증)

- PC 작업기가 NAS의 `historical_backfill` BODY/RULE 작업만 원자 선점해 기존 본문 추출·일반 평가·핵심문장·공급계약 규칙을 실행한다. 인증 API는 기사/본문/판정 revision과 작업 완료를 같은 트랜잭션에 기록하고 재전송·만료 시도를 구분한다. NAS 실시간 뉴스 작업은 그대로 유지한다. 새 NAS 빌드 식별자는 `2026.09.24-pc-news-preprocess-v1`이며 운영 배포·속도 실측 전까지 대량 실행하지 않는다.

## 2026-09-24 과거 연구 비용·개별 주식 입력·뉴스 수집기 안정화 (로컬 검증)

- 연구 비용에 `fixed_bps/v2`를 추가해 수수료 1.5bp와 매도세 20bp를 소수 bp 문자열로 고정한다. 기존 정수 `fixed_bps/v1` 요청은 호환해 읽고, 새 역사 구조 요청은 v2를 사용한다. 과거 1bp+18bp 요청은 성과 근거로 재사용하지 않는다.
- 과거 후보 원본의 `market_code`를 동결하고 명시적 개별 주식 투영을 추가했다. 새 3일 개발 입력은 TRAIN 42/42·VALIDATION 45/45에 결과일 분봉이 있지만 각 한 종목의 연속 1분쌍 공백 때문에 성과 실행 gate는 여전히 차단된다.
- PC 종목 뉴스 수집기에 단일 실행 파일 잠금을 적용하고 heartbeat 임시 파일을 실행별 고유 경로로 바꿨다. 중복 실행 스킵과 실제 재시작 뒤 heartbeat 갱신을 확인했다. 장시간 안정성은 계속 관찰한다.

## 2026-09-23 NAS 뉴스 원천·발행시각·저장분 페이지 조회 (로컬 검증)

- 네이버 검색 API와 네이버 증권 종목뉴스 목록을 운영 설정에서 각각 켜고 끄며, NAS 자율 수집은 현재 TOP20 종목만 대상으로 최근 2일 범위를 확인한다. 두 원천은 같은 제목·요약 판정과 원문 BODY/RULE 저장 경로를 사용한다.
- 원문에서 초 단위 발행시각을 확인하면 앱 조회용 `published_at`에 반영하고, 목록/API 발행시각은 원본 기사 revision과 `listing_published_at`에 보존한다. 원문 BODY 완료 뒤 일반 뉴스 판정과 핵심 문장도 NAS에 저장한다.
- NAS 연결 뉴스창은 저장된 기사만 200건씩 요청하고 스크롤 하단에서 다음 200건을 읽는다. NAS 모드 로컬 캐시는 200건 자동삭제를 적용하지 않는다. PC 직접 연결에는 설정 가능한 종목별 저장·표시 건수(50~1000건)를 적용한다. 날짜 공백은 수집 실패로 단정하지 않고 저장 기사 없음·수집 여부 미확인으로 표시한다.
- 빌드 식별자는 `2026.09.23-news-sources-stored-page-v1`. 관련 NAS 파일 9개를 SHA-256 일치로 동기화하고 기존 파일은 `X:\kiwoom-monitor-backups\20260923-news-sources-stored-page-v1`에 백업했다. 운영 컨테이너는 아직 이 빌드로 재빌드되지 않았으며 NAS 종목뉴스·원문 시각·페이지 UI 실운용 확인이 남아 있다.

## 2026-09-23 NAS 종목 뉴스 조회 복구

- NAS에 오늘 기사가 저장돼 있어도 앱의 최근 조회 이후 새 네이버 응답이 없으면 NAS가 0건을 반환하던 경로를 수정했다. 앱은 NAS에 최근 2일 표시 범위를 요청하고, NAS는 공급자 검색에만 자신의 마지막 확인 시각을 사용해 중복 요청을 줄인다. 신규 검색 결과와 저장 기사를 합쳐 반환하며 공급자 오류 때도 저장 기사를 우선 제공한다. NAS 빌드 식별자는 `2026.09.23-news-owner-cache-merge-v1`이다. 서버 파일 4개를 NAS 프로젝트와 SHA-256 일치로 동기화하고 기존 파일은 `X:\kiwoom-monitor-backups\20260923-news-owner-cache-merge-v1`에 보존했다. 운영 컨테이너 재빌드와 앱 재시작 후 실자료 확인은 남아 있다.

## 2026-09-23 과거 수집 재시도·해상도 경계 최적화

- NAS 순위 요청이 DB 기록 중인 이전 조회 뒤에서 기다리던 브로커 경계를 분리했다. 키움 REST 전송은 한 worker가 직렬 처리하고, 응답 기록·쿼리 캐시는 별도 직렬 worker가 처리한다. 사용자 조회의 저장 완료 대기 계약과 인증 전환 drain은 유지하며, `ka00198` 전송은 이전 응답의 DB 저장 완료를 기다리지 않는다. 1초 이상 저장 시 응답 기록과 캐시 저장 시간을 별도로 남긴다. NAS 빌드 식별자는 `2026.09.23-ranking-broker-persistence-v1`이다. 관련 NAS 소스 4개를 SHA-256 일치로 동기화하고 직전 파일을 `X:\kiwoom-monitor-backups\20260923-ranking-broker-persistence-v1`에 보존했다. 운영 컨테이너는 아직 이전 빌드다.

- CREON Plus의 06:05 연결 단절을 확인하고 재로그인 후 분봉·후보일 시총/주식수 수집을 이어받았다. 분봉 수집기의 종목별 범위 쿼리가 46GB 봉 인덱스를 전체 스캔하던 문제를 공급자 조건으로 바로잡아 해당 종목 검색으로 전환했다. 모니터는 연결 단절을 별도 상태로 표시하고 긴 실행 로그를 오류 칸에 복사하지 않는다.
- 시총/주식수 수집에서 CREON이 명시적으로 거부한 종목코드는 `unavailable`로 남겨 재시도를 반복하지 않는다. NAS 연구 스냅샷은 나머지 작업 완료 후에도 이 누락을 manifest에 `coverage_complete=false`와 건수로 공개하며 원오류 파일을 함께 보존한다.

- 대신 수집기의 프로세스당 종목 상한을 1,000개에서 10,000개로 늘리고 모니터 시작값도 맞췄다. 현재 약 3천 종목 원장을 한 실행에서 끝낼 수 있으며 작업 원장이 먼저 비면 즉시 종료한다.
- 대신 수집기 상한 변경 파일 6개를 NAS 프로젝트와 동기화해 SHA-256 불일치 0개를 확인했다. 기존 파일은 `X:\kiwoom-monitor-backups\20260923-031908-daishin-job-limit-v1`에 보존했다.
- 같은 언론사의 대기 원문이 작업자 스레드를 점유해 다른 언론사 요청까지 막던 원문 풀을 언론사별 대기열로 바꿨다. 언론사당 동시 요청 1개와 0.2초 간격은 유지하면서 16개 작업자가 서로 다른 언론사를 실제로 병렬 처리한다.
- 네이버 검색은 0.5초 간격에서 시간당 403 제한 대기가 반복돼 이론상 이득보다 강제 유휴시간이 커진 측정 결과를 반영해 기본 간격을 0.7초로 조정했다.
- 처리량 개선 파일 6개를 NAS 프로젝트와 동기화해 SHA-256 불일치 0개를 확인했다. 기존 파일은 `X:\kiwoom-monitor-backups\20260923-030147-news-throughput-v1`에 보존했다.
- 네이버 과거 검색이 HTTP 403/429를 받으면 이미 읽은 페이지를 버리고 같은 작업을 처음부터 다시 시작하던 경로를 수정했다. 검색 작업자 전체가 60초 제한 대기를 공유하고, 실패한 페이지만 최대 10회 재시도하며 제한 상태와 페이지를 heartbeat에 기록한다.
- 대신증권 후보 수집은 1분봉 최과거일의 전날을 5분봉 요청 종료일로 전달한다. 최신일부터 다시 받은 뒤 import에서 버리던 5분봉 중첩 구간을 공급자 요청 단계에서 제외한다.
- 대신 수집기를 재시작할 때마다 1억 건이 넘는 `market_bars` 전체를 다시 집계하던 초기화를 신규 작업 코드에만 한정했다. 기존 작업 원장이 있으면 전체 봉 집계를 생략하며, 중단된 부분 수집을 완료로 오인하지 않는다.
- 변경 파일 8개를 NAS 프로젝트와 동기화해 SHA-256 불일치 0개를 확인했다. 기존 파일은 `X:\kiwoom-monitor-backups\20260923-023816-collector-progress-v1`에 보존했다.

## 2026-09-23 과거 뉴스 사람 검토 화면

- 전략 연구 창의 `과거 뉴스 검토`에서 최신 불변 대기열과 호환 작업표를 자동으로 열고 기사 원문·요약·연결 종목·규칙 힌트를 보며 판정할 수 있다. 행 이동과 창 닫기 때 작업 CSV를 원자적으로 저장하고, 명시적으로 동결한 완료 행만 불변 결과가 된다. 저장된 테마 프로필 이름은 현재 프로필 목록에서 빠졌어도 보존한다.
- 추적 파일 968개를 NAS 프로젝트와 동기화해 SHA-256 불일치 0개를 확인했다. 기존 966개는 `X:\kiwoom-monitor-backups\20260923-001448-historical-news-review-ui-v1`에 보존했고 `.env`, `postgres-data`, `server-data`, `server-secrets`는 동기화 대상에서 제외했다. 로컬 데스크톱 연구 화면 변경이므로 NAS 컨테이너 재빌드는 필요하지 않다.
- `historical_news_event_split/v1`은 사람 검토에서 관련으로 확정한 기사만 `canonical_event_id`별로 묶고, 사건 전체를 검증된 최초 발행시각 순서대로 TRAIN·VALIDATION·봉인 OOS 중 한 구간에 배정한다. 무관·보류는 제외 수량으로 남기고 일부 검토 원천과 모델 학습 준비 상태를 명시한다. 결정 결과에는 분할 근거가 되는 기사 발행시각과 출처 provenance를 함께 보존한다.
...
- 방법 결과·채점 계약까지 포함한 추적 파일 982개를 NAS와 SHA-256 불일치 0개로 동기화했다. 직전 NAS 파일 977개는 `X:\kiwoom-monitor-backups\20260923-014830-news-method-evaluation-v1`에 보존했고 운영 데이터·비밀 경로는 제외했다.

## 2026-09-22 LLM 학습 준비 사례 계약

- `historical_news_review_queue/v1`은 같은 기사를 공급자·언론사·기사 식별자로 한 번만 검토하도록 묶고 종목·사례별 연결과 수집 provenance를 보존한다. 기존 비AI 판정은 검토 순서 힌트일 뿐 정답이 아니며 LLM 호출·사람 검토·가중치 학습 상태를 모두 false로 고정한다.
- 첫 대기열은 2,035개 종목-기사 관계를 고유 기사 1,422개로 줄여 613개의 반복 검토를 제거했다. 규칙상 우선 검토 힌트는 105개이며 전체 1,422개는 아직 검토 대기 상태다.
- `historical_news_review_workflow.py`는 우선 검토 CSV를 덮어쓰기 없이 내보내고, 관련·무관·보류 판정과 검토자·시각을 검증해 `historical_news_review_decisions/v1` 불변 결과로 가져온다. 관련 판정은 사건 ID가 필수이며 테마명을 입력하면 테마 프로필 이름도 필수다. 첫 105행 검토표는 생성했지만 사람 판정은 아직 0건이다.
- 워크플로 코드·문서 9개를 NAS 프로젝트에 동기화하고 기존 6개는 `X:\kiwoom-monitor-backups\20260922-213000-human-news-review-workflow-v1`에 보존했다. 추적 파일 966개의 SHA-256 불일치는 0개다. 편집 CSV는 아직 사람 판정이 없는 로컬 작업본이라 불변 NAS 결정 결과로 게시하지 않았다.
- 계약 코드·문서 9개를 NAS 프로젝트에 동기화하고 기존 6개는 `X:\kiwoom-monitor-backups\20260922-205900-historical-news-review-queue-v1`에 보존했다. 추적 파일 963개의 SHA-256 불일치는 0개다. 대기열은 NAS `server-data/historical-intelligence/v1/news-review-queues/historical-news-review-queue-578f89c206d709720184aa753649dc3840f949a066819b476fc27ce576311805`에 불변 게시하고 `latest.json`을 갱신했다.
