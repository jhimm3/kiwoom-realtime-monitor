...
- `historical_news_review_workflow.py`는 우선 검토 CSV를 덮어쓰기 없이 내보내고, 관련·무관·보류 판정과 검토자·시각을 검증해 `historical_news_review_decisions/v1` 불변 결과로 가져온다. 관련 판정은 사건 ID가 필수이며 테마명을 입력하면 테마 프로필 ID도 필수다. 첫 105행 검토표는 생성했지만 사람 판정은 아직 0건이다.
