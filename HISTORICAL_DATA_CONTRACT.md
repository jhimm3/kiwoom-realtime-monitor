...
기존 조회·재생·Paper·연구 엔진은 이미 구현된 범위에서 재사용한다. 다음 우선순위는 사용자 방향에 따른 입력 자료 확보다. 삼성전자와 일부 뉴스는 실제 확보했지만 아래 전체 범위 완료를 뜻하지 않는다.
...
2. 대신증권 API는 종목별 1분봉의 최초 완전 거래일을 경계로 삼고 그 이전 거래일만 5분봉을 저장한다. 두 기간은 중복되므로 전체 범위를 7년으로 합치거나 같은 날짜를 두 학습 사례로 세지 않는다. 원본 봉 단위·출처·거래소·세션·수정주가 여부·수집시각·공급자 원시 날짜/시각과 봉 시각 의미를 보존하고 실제 반환 범위와 결측을 검증한다.
3. 네이버 검색 API와 구분해 네이버 웹사이트 날짜 지정 검색의 원응답을 저장한다. 기사 원본·게시시각·최초 관측시각·종목 연결 근거와 URL별 확인 시도·상태를 남기며 과거 게시시각을 당시 수집기의 가용시각으로 소급하지 않는다. 당시 유효 상호를 기본 검색어로 쓰고 상호변경일 ±14일에는 구·신 이름을 모두 사용하며 이름 이력 근거를 보존한다. 원문 발행시각을 확인하지 못한 기사는 원자료로 보존하되 학습 적격으로 표시하지 않는다. 여러 후보일을 한 날짜 범위로 검색할 때도 검색 결과 또는 원문에서 확인한 실제 게시일로 일별 관계를 만들고, 날짜가 불명확한 결과를 범위 시작일에 임의 귀속하지 않는다. 범위 작업은 기존 일별 원장을 삭제하지 않고 구성 관계와 분할 이력을 보존한다.
