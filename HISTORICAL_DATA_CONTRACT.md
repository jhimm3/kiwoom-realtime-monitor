...
3. 네이버 검색 API와 구분해 네이버 웹사이트 날짜 지정 검색의 원응답을 저장한다. 기사 원본·게시시각·최초 관측시각·종목 연결 근거와 URL별 확인 시도·상태를 남기며 과거 게시시각을 당시 수집기의 가용시각으로 소급하지 않는다. 당시 유효 상호를 기본 검색어로 쓰고 상호변경일 ±14일에는 구·신 이름을 모두 사용하며 이름 이력 근거를 보존한다. 원문 발행시각을 확인하지 못한 기사는 원자료로 보존하되 학습 적격으로 표시하지 않는다.
