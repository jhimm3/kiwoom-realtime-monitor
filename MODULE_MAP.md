...
외부 `kiwoom_history_backfill`의 후보 DB·수집 스크립트는 앱 모듈이 아니다. [과거 수집 기획](docs/HISTORICAL_BACKFILL_PLAN.md)으로 원천별 표본을 확인한 뒤 기존 `research_data_source`, 뉴스 revision, 프로필 저장 경계에 연결한다. 새 파일명·테이블을 현재 존재하는 것으로 문서화하지 않는다.
