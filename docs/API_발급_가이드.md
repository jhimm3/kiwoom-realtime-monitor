...
이 앱은 키를 대신 발급하거나 개발자 계정의 사용료를 부담하지 않습니다. 필요한 기능만 선택해 각 공식 사이트에서 직접 발급하고 앱의 해당 설정 창에 입력하세요. 비밀키의 저장 위치는 실행 모드에 따라 다릅니다. 로컬 기능은 PC의 암호화 저장소를, NAS 중앙 기능은 서버의 암호화 비밀 저장소(`server-secrets`)를 사용합니다. 일반 설정 백업과 Google Drive 동기화에는 비밀키를 포함하지 않습니다. NAS 설치·인증 적용은 [시놀로지 서버 가이드](../deploy/synology/README.md)를 따르세요.
...
4. 로컬 연결에서는 앱의 `기본 설정 → 관리 → API 설정`에서 환경별 키를 입력하고 연결 테스트를 실행합니다. NAS 중앙 연결의 인증은 NAS 설정에서 해당 실전·모의 프로필을 확인하고 적용합니다.
...
이 절은 현재 앱의 뉴스 검색 API 기능을 위한 발급 방법입니다. 앞으로 확보하려는 네이버 증권 사이트의 과거 뉴스 수집과는 별개입니다.
...
4. 로컬 뉴스 기능은 뉴스창 톱니바퀴의 뉴스 API 설정에 두 값을 입력합니다. NAS 중앙 뉴스 기능은 NAS 설정의 해당 인증을 확인하고 적용합니다.
...
아래 뉴스·DART·AI 설정 안내는 로컬 기능의 입력 경로입니다. NAS에서 실행할 기능의 인증은 NAS 설정에서 확인·적용하고, 운영 설정도 NAS 중앙 설정을 기준으로 확인합니다.
