...
검색 페이지는 시작 간격 0.2초를 공유하는 4개 작업자로 응답 대기를 겹치며, 원문 확인은 같은 언론사 도메인에 한 요청만 허용하는 16개 작업자로 수행한다. 검색과 원문 처리는 한 작업 안에서도 파이프라인으로 진행한다. 검색 또는 원문 결과와 SQLite 저장은 메인 흐름에서 순서대로 확정해 재시작 원장과 중복 제거 계약을 유지한다.
