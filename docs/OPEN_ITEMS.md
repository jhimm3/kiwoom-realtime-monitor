...
| D03 | 부분 구현 | `historical_reconstruction/v1`과 기존 1분 전략 runner용 어댑터 구현. 후보일 장후 선택→다음 거래일 결과, 실제 수집시각/복원 clock, 사후 후보/당시 TOP20을 분리하고 순위 Factor를 차단. 2024~2026 세 날짜의 첫 다기간 불변 입력과 사례별 다음 거래일 중복 방지는 완료. 수집 확대 뒤 시간순 TRAIN/VALIDATION/OOS 고정과 대표 정책 비교는 남음 |
