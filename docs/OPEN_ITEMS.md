...
| D03 | 부분 구현 | `historical_reconstruction/v1`과 기존 1분 전략 runner용 어댑터 구현. 후보일 장후 선택→다음 거래일 결과, 실제 수집시각/복원 clock, 사후 후보/당시 TOP20을 분리하고 순위 Factor를 차단. 2024~2026 첫 다기간 입력, 사례별 다음 거래일 중복 방지, 데이터셋 hash에 결합한 TRAIN/VALIDATION/OOS 계획과 SEALED OOS 완료. 수집 확대 뒤 대표성 있는 새 분할 동결과 정책 비교는 남음 |
