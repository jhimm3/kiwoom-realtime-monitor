...
| D06 | 부분 구현 | 첫 50사례와 고유 기사 1,422개를 동결하고, 앱의 105행 사람 검토·불변 판정·사건 단위 시간순 분할·OOS 제외 공통 개발 입력 생성까지 연결함. 현재 사람 판정은 0건이라 실제 분할·개발 입력은 없음. 실제 관련성·사건·테마 검토, 첫 분할과 개발 입력 생성, 같은 입력에서 RAG와 미세조정 실행·평가, 로컬 LLM/Mac 작업자와 행동 EV 범위 결정이 남음 |
