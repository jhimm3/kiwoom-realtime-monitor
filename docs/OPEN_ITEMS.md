...
| D04 | 부분 구현 | 프로필별 별칭·분리 확장·재병합 금지 원장과 기존 병합/분리 동작, 파일 백업·NAS 전체 스냅샷 보존을 구현. 다음은 LLM이 사건 근거를 붙여 기존 대표 테마 연결/신규 후보를 제안하고 사용자가 검토하는 흐름. 동일 지역의 다른 사업은 지역명만으로 합치지 않음 |
