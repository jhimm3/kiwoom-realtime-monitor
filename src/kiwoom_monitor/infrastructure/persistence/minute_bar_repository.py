...
class MinuteBarRepository:
    ...
    def replace_market_index_minutes(
        ...
        values = tuple((minute.date().isoformat(), market, minute.isoformat(timespec="minutes"), *ohlc)
                       for (market, minute), ohlc in rows.items())
        ...
        try:
            connection.executemany(
                ...
                "trade_value_eok=COALESCE(excluded.trade_value_eok,market_index_minute_bars.trade_value_eok)", values,
            ); connection.commit()
    ...
    def replace_market_index_daily(self, market: str, rows: tuple[tuple[object, ...], ...]) -> None:
        ...
        try:
            connection.executemany(
                ...
                "close_value=excluded.close_value,volume=excluded.volume,trade_value_eok=excluded.trade_value_eok", values,
            ); connection.commit()
        finally: connection.close()
    ...
    def load_top20_statistics(self, days: int) -> tuple[
        ...
        try:
            hourly = connection.execute(
                ...
                "AND strftime('%w',trade_date) NOT IN ('0','6') GROUP BY substr(minute,12,2) "
