"""종목 기본정보 QThread의 생성과 신호 수명을 관리한다."""
...
from kiwoom_monitor.infrastructure.kiwoom_rest.fundamentals_worker import (
    FundamentalsWorker,
)


class FundamentalsWorkerController(QObject):
    ...
    def __init__(
        ...
        worker_factory: Callable[[tuple[str, ...]], FundamentalsWorker] | None = None,
    ) -> None:
    ...
    @property
    def worker(self) -> FundamentalsWorker | None:
        return self._worker
    ...
    def set_factory(
        self,
        worker_factory: Callable[[tuple[str, ...]], FundamentalsWorker] | None,
    ) -> None:
        self._worker_factory = worker_factory
    ...
    def start(
        ...
        if not isinstance(worker, FundamentalsWorker):
            self.failed.emit(
                "기본정보 작업 구성이 올바르지 않습니다. 프로그램을 다시 실행해 주세요."
            )
            worker.deleteLater()
            return False
        worker.received.connect(self.received.emit)
        worker.failed.connect(self.failed.emit)
        worker.completed.connect(lambda: self.completed.emit(followup_codes))
