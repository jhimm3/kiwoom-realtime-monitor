"""역사적 신고가 QThread의 생성과 신호 수명을 관리한다."""
...
class HistoricalHighWorkerController(QObject):
    ...
    def __init__(
        ...
        worker_factory: Callable[[tuple[str, ...]], HistoricalHighWorker] | None = None,
    ) -> None:
        super().__init__(parent)
        self._worker_factory = worker_factory
        self._worker: HistoricalHighWorker | None = None

    @property
    def worker(self) -> HistoricalHighWorker | None:
        return self._worker
    ...
    def set_factory(
        self,
        worker_factory: Callable[[tuple[str, ...]], HistoricalHighWorker] | None,
    ) -> None:
        self._worker_factory = worker_factory
    ...
    def start(self, codes: tuple[str, ...]) -> bool:
        ...
        if not isinstance(worker, HistoricalHighWorker):
            self.failed.emit(
                "역사적 신고가 작업 구성이 올바르지 않습니다. 프로그램을 다시 실행해 주세요."
            )
            worker.deleteLater()
            return False
