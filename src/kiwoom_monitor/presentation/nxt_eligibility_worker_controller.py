"""NXT 거래 가능 여부 QThread의 생성과 신호 수명을 관리한다."""
...
class NxtEligibilityWorkerController(QObject):
    ...
    def __init__(
        ...
        worker_factory: Callable[[tuple[str, ...]], NxtEligibilityWorker] | None = None,
    ) -> None:
        super().__init__(parent)
        self._worker_factory = worker_factory
        self._worker: NxtEligibilityWorker | None = None

    @property
    def worker(self) -> NxtEligibilityWorker | None:
        return self._worker
    ...
    def set_factory(
        self,
        worker_factory: Callable[[tuple[str, ...]], NxtEligibilityWorker] | None,
    ) -> None:
        self._worker_factory = worker_factory

    def start(self, codes: tuple[str, ...]) -> bool:
        if self.is_running:
            return True
        ...
        if not isinstance(worker, NxtEligibilityWorker):
            self.failed.emit(
                "NXT 확인 작업 구성이 올바르지 않습니다. 프로그램을 다시 실행해 주세요."
            )
            worker.deleteLater()
            return False
